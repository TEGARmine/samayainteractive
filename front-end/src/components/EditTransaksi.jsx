...
import { getTransaksiById, updateTransaksi, getBarang } from "../services/api";
...
function EditTransaksi() {
  ...
  useEffect(() => {
    const fetchHargaSatuan = async () => {
      try {
        ...
        console.error("Error setting harga satuan:", error);
        setError(error.message);
      }
    };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await updateTransaksi(id, transaksi);
      navigate("/transaksi");
    } catch (error) {
      setError(error.response?.data?.message || "Failed to update transaksi");
    }
  };
